...
const LedgerReports = () => {
  ...
  const [ledgerReportsData, setLedgerReportsData] = React.useState([]);
  ...
  const getStatusWiseFilteredData = async () => {
    ...
    // setPage(1);
    setLoading(true);
    var query = `${config.REACT_APP_API_URL_ADMIN}/accounts/reports/ledger?limit=${limit}&page=${page}`;
    ...
    try {
      ...
      if (res.isSuccess) {
        setLoading(false);
        setLedgerReportsData(res?.result?.results);
      }
    } catch (err) {
      console.log(err);
    }
  ...
  const getLedgerReports = async () => {
    setLoading(true);
    const res = await getLedgerListData(10, 1);
    if (res.isSuccess) {
      setLoading(false);
      setLedgerReportsData(res?.result?.results);
    }
  ...
  //   setLimit(10);
  //   try {
  //     const res = await getLedgerListData(10, page + 1);
  ...
  const loadMoreData = async () => {
    ...
    }/accounts/reports/ledger?limit=${limit}&page=${page + 1}`;
    if (start_date) {
    ...
    <SafeAreaView>
      <ScrollView>
        ...
        <View style={styles.container}>
          ...
          {ledgerReportsData?.length > 0 ? (
            <AccordionList
              data={ledgerReportsData ? ledgerReportsData : []}
              customTitle={item => (
                <Text>
                  <B>User Name: </B>
                  {item?.person}
