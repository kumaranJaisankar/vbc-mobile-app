...
const LeadReports = () => {
  ...
  const [LeadReportsData, setLeadReportsData] = React.useState([]);
  const [isLoading, setLoading] = React.useState(true);
  const [rgbHeight, setRgbHeight] = React.useState(470);
  ...
  const getLeadReports = async () => {
    var query = `${config.REACT_APP_API_URL}/radius/lead/reports?limit=10&page=1&created=${start_date}&created_end=${end_date}`;
    setLoading(true);
    const res = await getFilterCustomerData(query);
    console.log(res, 'lead res');
    if (res.isSuccess) {
      setLoading(false);
      setLeadReportsData(res?.result?.results);
    }
  };
  ...
  const getStatusWiseFilteredData = async () => {
    ...
    setPage(1);
    setLoading(true);
    var query = `${config.REACT_APP_API_URL}/radius/lead/reports?limit=${limit}&page=${page}`;
    ...
    if (leadStatusId) {
      if (leadStatusId === 'ALL') {
        ...
        query = query + `&status=${leadStatusId}`;
      }
    }
    if (assignToId) {
      query = query + `&assigned_to=${assignToId}`;
    }
    try {
      setLoading(true);
      const res = await getFilterCustomerData(query);
  ...
  const loadMoreData = async () => {
    ...
    }/radius/lead/reports?limit=${limit}&page=${page + 1}`;
    if (start_date) {
      query = query + `&created=${start_date}`;
    }
    if (end_date) {
    ...
    <SafeAreaView>
      <ScrollView>
        ...
        <View style={styles.container}>
          ...
          {LeadReportsData?.length > 0 ? (
            <AccordionList
              data={LeadReportsData ? LeadReportsData : []}
              customTitle={item => (
                <Text>
                  <B>Name: </B>
              ...
              customBody={item => (
                <Text>
                  ...
                  {'\n'}
                  <B>Address: </B>
                  {`${item?.house_no},${item?.street},${item?.landmark},${item?.city},${item?.district},${item?.state},${item?.country},${item?.pincode}`}
                  {'\n'}
                  {'\n'}
                  <B>Assigned To: </B>
                  {item?.assigned_to}
