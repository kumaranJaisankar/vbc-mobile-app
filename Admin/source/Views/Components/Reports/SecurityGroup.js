...
const SecurityGroup = () => {
  ...
  const [rgbHeight, setRgbHeight] = React.useState(300);
  ...
  const [securityDepositID, setSecurityDepositID] = useState('All');
  ...
  const getReports = async () => {
    // var query = `${config.REACT_APP_API_URL_BILLING}/payment/enh/list?limit=10&page=1&security_deposit_refund=True&start_date=${start_date}&end_date=${end_date}`;
    var query = `${config.REACT_APP_API_URL_BILLING}/payment/security/deposit/reports/list?limit=10&page=1&security_deposit_refund=All&start_date=${start_date}&end_date=${end_date}`;
    try {
      setLoading(true);
      const res = await getSecurityGroupReports(query);
      console.log('🚀 ~ file: SecurityGroup.js:16 ~ getReports ~ res:', res);
      if (res.isSuccess) {
        setLoading(false);
        setSecurityGroup(res?.result?.results);
      }
    } catch (err) {
      console.log(err);
    }
  ...
  const getStatusWiseFilteredData = async () => {
    ...
    setPage(1);
    setLoading(true);
    // var query = `${config.REACT_APP_API_URL_BILLING}/payment/enh/list?limit=${limit}&page=${page}&security_deposit_refund=${securityDepositID}`;
    var query = `${config.REACT_APP_API_URL_BILLING}/payment/security/deposit/reports/list?limit=${limit}&page=${page}&security_deposit_refund=${securityDepositID}&start_date=${start_date}&end_date=${end_date}`;
    if (start_date) {
  ...
  console.log('value here', start_date);
  console.log('value here2', end_date);
  const loadMoreData = async () => {
    setPage(page + 1);
    setLimit(10);
    setLoading(true);
    // var query = `${config.REACT_APP_API_URL_BILLING}/payment/enh/list?limit=${limit}&page=${
    //   page + 1
    // }&security_deposit_refund=${securityDepositID}`;

    var query = `${
      config.REACT_APP_API_URL_BILLING
    }/payment/security/deposit/reports/list?limit=${limit}&page=${
      page + 1
    }&security_deposit_refund=${securityDepositID}&start_date=${start_date}&end_date=${end_date}`;
  ...
  const security_deposit_list = [
    {name: 'All', id: 'All'},
    {name: 'Security Deposit Refund', id: 'True'},
    {name: 'Security Deposit', id: 'False'},
  ];
                  ...
                  <View
                    ...
                    <View style={{flex: 2}}>
                      <Dropdown
                        ...
                        placeholderStyle={styles.placeholderStyle}
                        selectedTextStyle={styles.selectedTextStyle}
                        data={security_deposit_list}
                        maxHeight={200}
                        labelField="name"
                        valueField="id"
                        placeholder="Security Deposit Refund"
