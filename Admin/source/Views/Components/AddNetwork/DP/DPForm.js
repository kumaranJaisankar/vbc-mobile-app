...
const DPForm = props => {
  ...
  const [noOfPorts, setNoOfPorts] = useState(0);
  ...
  const [serialNo, setSerialNo] = useState({ name: '' });
  ...
  const onAddClick = async () => {
    if (
      ...
      setLoading({ spinner: true, spinnerText: 'Creating DP...' });
      try {
        if (selectDevice === 1) {
          var response = await addPDP(body);
        } else {
          var response = await addCDP(body);
        }
        console.log(
...
export default connect(mapStateToProps)(DPForm);
