...
// const ConnectedCPE = () => {
//   return (
//     <div>ConnectedCPE</div>
//   )
// }

// export default ConnectedCPE
...
import {getCPEInfo, getOLTInfo, getPDPInfo} from '../../services/MainService';
import DialogView from '../../Common/DialogView';
const {width, height} = Dimensions.get('window');

const ConnectedCPE = props => {
  ...
  const cpeDetails = async deviceId => {
    setLoading(true);
    const res = await getCPEInfo(deviceId);
    console.log('🚀 ~ file: ConnectedCPE.js:46 ~ pdpDetails ~ res:', res);
    if (res?.isSuccess) {
      setLoading(false);
  ...
  return (
    <View style={{flex: 1, backgroundColor: 'white'}}>
      <HeaderView1
        username={`Details Of CPE - ${detailsInfo?.hardware_name}`}
        showHeaderBack
        onBackClick={() => {
          navigation.goBack();
        }}
      ...
      <ScrollView>
        ...
        <View
          ...
          <Text style={{color: 'black', fontWeight: 'bold'}}>
            {detailsInfo?.hardware_name} Is Connected To:
          </Text>
          <View
            style={{
              borderWidth: 1,
            ...
            <Text
              ...
              Optical Line Terminal: {detailsInfo?.parent_childdp?.name}
            </Text>
            ...
            <View
              ...
              <View style={{width: width * 0.25, height: height * 0.1}}>
                <View
                  ...
                  <Text style={{fontSize: height * 0.015, color: 'black'}}>
                    {detailsInfo?.parent_childdp?.serial_no}
                  </Text>
                  <Text style={{fontSize: height * 0.015, color: 'black'}}>
                    {detailsInfo?.parent_childdp?.available_ports}
                  </Text>
                </View>
              </View>
            </View>
              ...
              <View style={{width: width * 0.7, height: height * 0.1}}>
                <View
                  ...
                  <Text style={{color: 'black'}}>
                    {detailsInfo?.parent_childdp?.house_no},
                    {detailsInfo?.parent_childdp?.street},
                    {detailsInfo?.parent_childdp?.landmark},
                    {detailsInfo?.parent_childdp?.city},
                    {detailsInfo?.parent_childdp?.district},{' '}
                    {detailsInfo?.parent_childdp?.country},
                    {detailsInfo?.parent_childdp?.pincode}
                  </Text>
                </View>
              </View>
            </View>
