...
import {getOLTInfo, getPDPInfo} from '../../services/MainService';
import DialogView from '../../Common/DialogView';
const {width, height} = Dimensions.get('window');

const ConnectedDevicePDP = props => {
  ...
  const pdpDetails = async deviceId => {
    setLoading(true);
    const res = await getPDPInfo(deviceId);
    console.log('🚀 ~ file: ConnectedDevicePDP.js:37 ~ pdpDetails ~ res:', res);
    if (res?.isSuccess) {
      setLoading(false);
  ...
  return (
    <View style={{flex: 1, backgroundColor: 'white'}}>
      <HeaderView1
        username={`Details Of Parent DP - ${detailsInfo?.name}`}
        showHeaderBack
        onBackClick={() => {
          navigation.goBack();
        }}
...
export default ConnectedDevicePDP;
